...
const kandjiVulnerabilityLicenseNote =
  "Kandji Vulnerability Management is not enabled for this tenant.";

type KandjiPhase = "validate" | "execute";
type KandjiActionHandler = (
...
export const kandjiActionHandlers: ProviderActionHandlers<
  "kandji",
  KandjiActionHandler
> = {
  ...
  async get_blueprint(input, context) {
    ...
    return {
      blueprint: normalizeBlueprint(
        requireObject(payload, "Kandji blueprint response"),
      ),
    };
  },
  ...
  async list_devices(input, context) {
    ...
    const payload = await executeKandjiGet(context, "/api/v1/devices", {
      ...
      filevault_enabled:
        filevaultEnabled === undefined ? undefined : String(filevaultEnabled),
      limit: optionalInteger(input.limit),
      offset: optionalInteger(input.offset),
    });
  ...
  async get_device_apps(input, context) {
    const payload = await executeKandjiGet(
      context,
      `${devicePath(input.deviceId)}/apps`,
    );
  ...
  async get_device_status(input, context) {
    const payload = await executeKandjiGet(
      context,
      `${devicePath(input.deviceId)}/status`,
    );
  ...
  async get_device_activity(input, context) {
    const payload = await executeKandjiGet(
      context,
      `${devicePath(input.deviceId)}/activity`,
      {
    ...
    const envelope =
      optionalRecord(
        requireObject(payload, "Kandji device activity response").activity,
      ) ?? {};
...
export async function validateKandjiCredential(
  ...
  const record = requireObject(
    payload,
    "Kandji credential validation response",
  );
...
export function normalizeKandjiApiUrl(value: unknown): string {
  ...
  if (!isAllowedKandjiApiHost(url.hostname)) {
    throw new ProviderRequestError(
      400,
      "apiUrl host must end with .api.kandji.io or .api.eu.kandji.io",
    );
  }
  ...
  if (url.pathname !== "/") {
    throw new ProviderRequestError(
      400,
      "apiUrl must be the Kandji API root URL",
    );
  }
...
async function requestKandjiJson(input: {
  ...
  return runProviderRequest(
    { signal: input.signal, label: "Kandji" },
    async (signal) => {
      const response = await input.fetcher(
        buildKandjiUrl(input.apiUrl, input.path, input.query),
        {
          method: "GET",
      ...
      if (!response.ok) {
        throw createKandjiError(
          response.status,
          payload,
          input.phase,
          input.notFoundAsInvalidInput,
        );
      }
...
function isAllowedKandjiApiHost(hostname: string): boolean {
  ...
  return (
    hasTenantSuffix(lower, ".api.kandji.io") ||
    hasTenantSuffix(lower, ".api.eu.kandji.io")
  );
}
...
function createKandjiError(
  ...
  const message =
    extractKandjiErrorMessage(payload) ??
    `Kandji request failed with status ${status}`;
...
function normalizeKandjiList(value: unknown): {
  ...
  return {
    items,
    total:
      optionalInteger(record.count) ?? optionalInteger(record.total) ?? null,
    pagination: normalizePagination(record),
  };
}

function vulnerabilityPagination(
  input: Record<string, unknown>,
...
function enrichDeviceModel(details: unknown, summary: unknown): unknown {
  ...
  if (
    ...
    !marketingModel ||
    (currentModel &&
      !/^(Mac|iPhone|iPad|AppleTV|Apple TV|VisionPro|Vision Pro)$/iu.test(
        currentModel,
      ))
  ) {
    return details;
...
function normalizeBlueprintList(value: unknown) {
  ...
  return value.map((item) =>
    normalizeBlueprint(requireObject(item, "Kandji blueprint record")),
  );
}
...
function normalizeUserList(value: unknown) {
  ...
  return value.map((item) =>
    normalizeUser(requireObject(item, "Kandji user record")),
  );
}
